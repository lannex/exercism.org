...
abstract class ForthTest {
    ...
    class Subtraction : ForthTest() {
        @Test
        fun `can subtract two numbers`() = assertEquals(
            listOf(-1),
            Forth().evaluate("3 4 -")
        )
    ...
    class Multiplication : ForthTest() {
        @Test
        fun `can multiply two numbers`() = assertEquals(
            listOf(8),
            Forth().evaluate("2 4 *")
        )
    ...
    class Division : ForthTest() {
        @Test
        fun `can divide two numbers`() = assertEquals(
            listOf(4),
            Forth().evaluate("12 3 /")
        )

        @Test
        fun `performs integer division`() = assertEquals(
            listOf(2),
            Forth().evaluate("8 3 /")
        )
    ...
    class CombinedArithmetic : ForthTest() {
        @Test
        fun `addition and subtraction`() = assertEquals(
            listOf(-1),
            Forth().evaluate("1 2 + 4 -")
        )

        @Test
        fun `multiplication and division`() = assertEquals(
            listOf(2),
    ...
    class Dup : ForthTest() {
        @Test
        fun `copies a value on the stack`() = assertEquals(
            listOf(1, 1),
            Forth().evaluate("1 dup")
        )

        @Test
        fun `copies the top value on the stack`() = assertEquals(
            listOf(1, 2, 2),
    ...
    class UserDefinedWords : ForthTest() {
        ...
        fun `can use different words with the same name`() =
            assertEquals(listOf(5, 6), Forth().evaluate(": foo 5 ;", ": bar foo ;", ": foo 6 ;", "bar foo"))
    ...
    class CaseInsensitivity : ForthTest() {
        @Test
        fun `DUP is case-insensitive`() = assertEquals(
            listOf(1, 1, 1, 1),
            Forth().evaluate("1 DUP Dup dup")
        )

        @Test
        fun `DROP is case-insensitive`() = assertEquals(
            listOf(1),
            Forth().evaluate("1 2 3 4 DROP Drop drop")
        )

        @Test
        fun `SWAP is case-insensitive`() = assertEquals(
            listOf(2, 3, 4, 1),
            Forth().evaluate("1 2 SWAP 3 Swap 4 swap")
        )

        @Test
        fun `OVER is case-insensitive`() = assertEquals(
            listOf(1, 2, 1, 2, 1),
